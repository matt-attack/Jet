...
void StructExpression::AddConstructorDeclarations(Type* str, CompilerContext* context)
{
	...
	if (has_constructor == false)
	{
		...
		fun->return_type_ = context->root->VoidType;
		fun->arguments_ = { { str->GetPointerType(), "this" } };
		fun->type_ = (FunctionType*)-1;//indicates this is autogenerated
...
void StructExpression::AddConstructors(CompilerContext* context)
{
	...
	for (auto ii : str->data->functions)
	{
        ...
        if (ii.second->is_extern_)
        {
            continue;
        }

		//need to identify if its a autogenerated fun
		if (ii.second->expression_ == 0 && ii.second->type_ == (FunctionType*)-1 
			&& ii.second->name_.length() > strname.length() + 2 
			&& ii.second->name_.substr(2, strname.length()) == strname)
		{
			//its probably a constructor/destructor we need to fill in
			auto res = ii.second->name_.find('~');
			if (res != -1)
			{
				//destructor
				...
				ii.second->f_ = function->function->f_;
				...
				//alloc args
				auto AI = function->function->f_->arg_begin();
				for (unsigned Idx = 0, e = argsv.size(); Idx != e; ++Idx, ++AI) {
					...
					auto local = context->root->debug->createAutoVariable(
						function->function->scope_, aname, context->root->debug_info.file, this->token.line,
					...
					llvm::Instruction *call = context->root->debug->insertDeclare(
						...
						llvm::DebugLoc::get(this->token.line, this->token.column, function->function->scope_), context->root->builder.GetInsertBlock());
					call->setDebugLoc(llvm::DebugLoc::get(this->token.line, this->token.column, function->function->scope_));
				...
				for (auto ii : str->data->struct_members)
				{
					if (ii.type->type == Types::Struct)
					{
						...
						{
							if (iii->second->arguments_.size() == 1)
							{
								...
								context->root->builder.CreateCall(iii->second->f_, gep);
							}
						}
					}
					i++;
			...
			else//constructor
			{
				...
				for (unsigned Idx = 0, e = argsv.size(); Idx != e; ++Idx, ++AI) {
					...
					auto D = context->root->debug->createAutoVariable(function->function->scope_, aname, unit, this->token.line,
						argsv[Idx].first->GetDebugType(context->root));

					llvm::Instruction *Call = context->root->debug->insertDeclare(
						AI, D, context->root->debug->createExpression(), llvm::DebugLoc::get(this->token.line, this->token.column, function->function->scope_), context->root->builder.GetInsertBlock());
					Call->setDebugLoc(llvm::DebugLoc::get(this->token.line, this->token.column, function->function->scope_));
		...
		else if (ii.second->name_ == "__" + str->data->name + "_" + strname)
		{
			auto rp = context->root->builder.GetInsertBlock();
			auto dp = context->root->builder.getCurrentDebugLocation();
            ...
            auto iter = ii.second->f_->getBasicBlockList().begin()->begin();
			...
			for (auto iip : str->data->struct_members)
			{
				if (iip.type->type == Types::Struct)
				{
					...
					for (auto iii = range.first; iii != range.second; iii++)
					{
						if (iii->second->arguments_.size() == 1)
						{
							auto myself = ii.second->context_->Load("this");
							std::vector<llvm::Value*> iindex = { context->root->builder.getInt32(0), context->root->builder.getInt32(i) };
				...
				else if (iip.name == "__vtable")//is vtable)
				{
					...
					CValue vtable = ii.second->context_->GetVariable("__" + this->name.text + "_vtable");
		...
		else if (ii.second->name_ == "__" + str->data->name + "_~" + strname)
		{
            ...
            auto& bb = ii.second->f_->getBasicBlockList().back();
